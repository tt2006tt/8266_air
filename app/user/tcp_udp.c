...
os_timer_t uart_main;//用串口轮询
Typdef_DataFormat DataFormat = {0};	//初始化数据结构体
Typdef_ProgramPara ProgramPara = {0}; //初始化结构体
Typdef_PmData PmData = {0};
Typdef_MapFormat  MapFormat = {0};
Typdef_UartFormat UartFormat = {0};//初始化串口结构体
uint8_t UartPackege[DataMax] = {0};
uint8_t UartRecPackege[DataMax] = {0};
...
uint8_t uart_cnt = 0;
uint8_t uart_dst_addr = 0;//用来保存模块发上来的地址

extern uint8_t	gassmartconfig;
uint8_t uart_onoff = 1;
...
websocket_tcp_recv_cb(void *arg, char *pusrdata, unsigned short length)
{
   //received some data from tcp connection
	...
	if(strstr(pusrdata,"HTTP/1.1 101"))
	{
		...
		if(ProgramPara.Dev_ID[0] == 'D' && ProgramPara.Dev_ID[1] == 'A'){
